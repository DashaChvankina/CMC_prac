...
class KNNClassifier:
    ...
    def predict(self, X):
        if self.weights:
            ...
            ar_y = np.take(self.y, ind)  # отклики ближайших соседей
            dist += 1e-5
            weigh = 1 / dist  # веса
            labels = np.unique(ar_y.astype('str'), axis=1)  # уникальные отклики для каждой строки
            list_sum = []
            for row in labels.T:
            ...
            ar_y = np.take(self.y, ind)  # отклики ближайших соседей
            labels = np.unique(ar_y.astype('str'), axis=1)
            list_sum = []
            for row in labels.T:
